...
describe('Advanced Features (AsyncFlowState 2.0.0)', () => {
  ...
  describe('Ghost Workflows', () => {
    it('should maintain idle state during ghost execution', async () => {
      const action = vi.fn().mockImplementation(() => new Promise(res => setTimeout(() => res('done'), 100)));
  ...
  describe('Dead Letter Queue (DLQ)', () => {
    it('should capture failed actions in the DLQ', async () => {
      ...
      const flow = new Flow(action, { deadLetter: true, retry: { maxAttempts: 1 } });
  ...
  describe('AI Utilities', () => {
    it('should auto-heal on failure', async () => {
      ...
      const workflow = withAutoHealing(new Flow(failingAction, { retry: { maxAttempts: 1 } }), { 
