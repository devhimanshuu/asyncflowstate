 ...
 */
export function calculateDeepDiff(oldObj: any, newObj: any, path: string[] = []): JsonDiff[] {
  ...
  if (typeof oldObj === 'object') {
    ...
    newKeys.forEach(key => {
      if (!(key in oldObj)) {
        diffs.push({ type: 'ADDED', path: [...path, key], newValue: newObj[key] });
      } else {
        diffs.push(...calculateDeepDiff(oldObj[key], newObj[key], [...path, key]));
      }
    });
    ...
    oldKeys.forEach(key => {
      if (!(key in newObj)) {
        diffs.push({ type: 'REMOVED', path: [...path, key], oldValue: oldObj[key] });
      }
    });
  } else {
    diffs.push({ type: 'UPDATED', path, oldValue: oldObj, newValue: newObj });
